// Copyright Bartosz Pietryka. All Rights Reserved.
...
// This file was forked from https://github.com/aws/aws-signer-notation-plugin/blob/main/internal/logger/logger.go
...
func New() (*debugLogger, error) {
	...
	path := filepath.Join(cfgDir, "notation-piv-plugin", "plugin.log")
	...
	dl.Debugln("-----------------------------------------------------------------------------------------")
	dl.Debugf("Logs from execution of PIV signer plugin version: %s\n", version.GetVersion())
	return dl, nil
}
...
func (l *debugLogger) logf(levelPrefix, format string, args ...interface{}) {
	if l.file != nil {
		_, _ = fmt.Fprintf(l.file, "\n%s [%s] "+format, append([]interface{}{time.Now().Format(time.RFC3339Nano), levelPrefix}, args...)...)
	}
}
